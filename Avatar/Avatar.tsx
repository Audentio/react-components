...
import { getEntityHref } from '@audentio/utils/src/getEntityHref';
...
export class Avatar extends Component<AvatarProps> {
    ...
    getEntityAbbreviation(entity) {
        if (entity.__typename === 'EventRoster') {
            if (entity.abbreviation && entity.abbreviation.length) return entity.abbreviation;
            if (entity.organization) return entity.organization.abbreviation;
        }
    ...
    getEntityImage(entity) {
        ...
        if (entity.avatar) {
            ...
            if (size === 'stretch' || size === 'lg') return entity.avatar.l;
            if (size === 'xs' || size === 'sm' || size === 'us') return entity.avatar.s;
    ...
    getAbbrFontSize(size: string, abbr?: string) {
        if (!abbr || !abbr.length || size === 'stretch') return 14;
    ...
    getEntityColors(entity, image): React.CSSProperties {
        if (image) {
            return {
    ...
    public render(): React.ReactNode {
        const { href, onClick, border, outline, size, className, entity = {} } = this.props;

        if (entity) {
            const image = this.getEntityImage(entity);
            const avatar_href = href !== undefined ? href : getEntityHref(entity);
            const abbr = size === 'us' ? '' : this.getEntityAbbreviation(entity);
            const Element: React.ReactNode = avatar_href && !onClick ? Anchor : 'span';

            return (
                // @ts-ignore
                <Element
                    href={onClick ? undefined : avatar_href || undefined}
                    onClick={onClick}
                    style={{
