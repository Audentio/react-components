...
export class Form extends Component {
    constructor() {
        ...
        this.validator = {};
        this.__formRef = React.createRef();
         ...
         */
        this.validate = throttle(async (value) => {
    ...
    renderFields(children, initial) {
        ...
        return React.Children.map(children, (child) => {
            ...
            if (child.props.name) {
                ...
                const value = formValue[child.props.name] === undefined ? child.props.initialValue : formValue[child.props.name];
                if (schema) {
            ...
            if (child.type &&
                child.type.displayName === this.props.buttonTypeName &&
                child.props.type === 'submit') {
                return React.cloneElement(child, {
                    disabled: (!this.state.isValid && this.state.touchedAll) || this.props.disabled || child.props.disabled,
     ...
     */
    getSubmitValue(__formValue, __children) {
    ...
    render() {
        ...
        if (formless) {
            return (<div style={inlineStyle} className={className}>
                    {this.renderFields(children, true)}

                    {debug && __DEV__ && this.renderDebugState()}
                </div>);
        }
        return (<form ref={this.__formRef} style={inlineStyle} className={className} noValidate onReset={this.onReset} onSubmit={this.onSubmit} autoComplete={autoComplete} {...passProps(rest)}>
                {this.renderFields(children, true)}
